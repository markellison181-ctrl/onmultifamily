          ...
          <div className="relative z-10 flex flex-col gap-6 sm:flex-row sm:items-center">
            ...
            {mcStatus === 'success' ? (
              ...
              <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 flex-1 sm:max-w-lg">
                ...
                />
                <button type="submit" disabled={mcStatus === 'loading'} className="bg-gradient-to-r from-gold to-gold-light text-navy text-[12px] tracking-[0.15em] uppercase font-bold px-8 py-4 hover:shadow-[0_0_30px_rgba(201,168,76,0.3)] transition-all duration-500 whitespace-nowrap disabled:opacity-50">
                  {mcStatus === 'loading' ? 'Subscribing...' : 'Subscribe'}
                </button>
                {mcStatus === 'error' && <p className="text-red-400 text-[12px] self-center">{mcMessage}</p>}
              </form>
            )}
          </div>
        </div>
